...
onMounted(() => {
  ...
  camera.zoom = 2; // Increase the zoom level
camera.updateProjectionMatrix(); // Update the camera's projection matrix
  ...
  document.getElementById('bg-canvas-3').appendChild(renderer.domElement)
  ...
  loader.load('/public/tesseract.glb', (gltf) => {
    model = gltf.scene
    scene.add(model)
  ...
  document.addEventListener('mousemove', (e) => {
    target.x = (e.clientX / window.innerWidth - 0.5) * 5
